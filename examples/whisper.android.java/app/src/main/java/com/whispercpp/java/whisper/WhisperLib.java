...
public class WhisperLib {
  ...
  static {
    ...
    CDELog.j(LOG_TAG, "Primary ABI: " + Build.SUPPORTED_ABIS[0]);
    ...
    if (WhisperUtils.isArmEabiV7a()) {
      ...
      if (cpuInfo != null) {
        ...
        if (cpuInfo.contains("vfpv4")) {
          CDELog.j(LOG_TAG, "CPU supports vfpv4");
          loadVfpv4 = true;
        }
      }
      ...
      String cpuInfo = WhisperUtils.cpuInfo();
      if (cpuInfo != null) {
        CDELog.j(LOG_TAG, "CPU info: " + cpuInfo);
        if (cpuInfo.contains("fphp")) {
          CDELog.j(LOG_TAG, "CPU supports fp16 arithmetic");
          loadV8fp16 = true;
        }
      }
    }
    ...
    if (loadVfpv4) {
      CDELog.j(LOG_TAG, "Loading libwhisper_vfpv4.so");
      System.loadLibrary("whisper_vfpv4");
    } else if (loadV8fp16) {
      CDELog.j(LOG_TAG, "Loading libwhisper_v8fp16_va.so");
      System.loadLibrary("whisper_v8fp16_va");
    } else {
      CDELog.j(LOG_TAG, "Loading libwhisper.so");
