...
int whisper_pcm_to_mel(struct whisper_context * ctx, struct whisper_state * state, const float * samples, int n_samples, int n_threads) {
    if (!log_mel_spectrogram(*ctx, *state, samples, n_samples, WHISPER_SAMPLE_RATE, WHISPER_N_FFT, WHISPER_HOP_LENGTH, WHISPER_N_MEL, n_threads, ctx->model.filters, false, state->mel)) {
...
int whisper_pcm_to_mel_phase_vocoder(struct whisper_context * ctx, struct whisper_state * state, const float * samples, int n_samples, int n_threads) {
    if (!log_mel_spectrogram(*ctx, *state, samples, n_samples, WHISPER_SAMPLE_RATE, 2*WHISPER_N_FFT, 2*WHISPER_HOP_LENGTH, WHISPER_N_MEL, n_threads, ctx->model.filters, true, state->mel)) {
        fprintf(stderr, "%s: failed to compute mel spectrogram\n", __func__);
        return -1;
    }
...
int whisper_encode(struct whisper_context * ctx, struct whisper_state * state, int offset, int n_threads) {
    if (!whisper_encode(*ctx, *state, offset, n_threads)) {
...
int whisper_decode(struct whisper_context * ctx, struct whisper_state * state, const whisper_token * tokens, int n_tokens, int n_past, int n_threads) {
    // TODO: add selected_decoder_id to context
    const int selected_decoder_id = 0;

    if (!whisper_decode(*ctx, *state, state->decoders[selected_decoder_id], tokens, n_tokens, n_past, n_threads)) {
        fprintf(stderr, "%s: failed to eval\n", __func__);
        return 1;
    }
...
int whisper_lang_auto_detect(
    ...
    // run the encoder
    if (whisper_encode(ctx, state, seek, n_threads) != 0) {
    ...
    if (whisper_decode(ctx, state, prompt.data(), prompt.size(), 0, n_threads) != 0) {
...
float * whisper_get_logits(struct whisper_state * state) {
...
void whisper_print_timings(struct whisper_context * ctx, struct whisper_state * state) {
...
void whisper_reset_timings(struct whisper_state * state) {
    state->t_sample_us = 0;
    state->t_encode_us = 0;
    state->t_decode_us = 0;
}
...
int whisper_full_with_state(
    ...
    if (params.speed_up) {
        ...
        if (whisper_pcm_to_mel(ctx, state, samples, n_samples, params.n_threads) != 0) {
            fprintf(stderr, "%s: failed to compute log mel spectrogram\n", __func__);
            return -2;
        }
    }
    ...
    if (params.language == nullptr || strlen(params.language) == 0 || strcmp(params.language, "auto") == 0) {
        ...
        const auto lang_id = whisper_lang_auto_detect(ctx, state, 0, params.n_threads, probs.data());
        if (lang_id < 0) {
    ...
    while (true) {
        ...
        if (!whisper_encode(*ctx, *state, seek, params.n_threads)) {
            fprintf(stderr, "%s: failed to encode\n", __func__);
            return -6;
        }
        ...
        for (int it = 0; it < (int) temperatures.size(); ++it) {
            ...
            // run whisper_decoder() only for decoder 0 and copy the results for the other decoders
            {
                ...
                if (!whisper_decode(*ctx, *state, state->decoders[0], prompt.data(), prompt.size(), 0, params.n_threads)) {
                    fprintf(stderr, "%s: failed to decode\n", __func__);
                    return -7;
                }
            ...
            for (int i = 0, n_max = whisper_n_text_ctx(ctx)/2 - 4; i < n_max; ++i) {
                ...
                for (int j = 0; j < n_decoders_cur; ++j) {
                    ...
                    if (!whisper_decode(*ctx, *state, decoder, decoder.tokens_tmp.data(), decoder.tokens_tmp.size(), decoder.kv_self.n, params.n_threads)) {
                        fprintf(stderr, "%s: failed to decode\n", __func__);
                        return -8;
                    }
...
int whisper_full_n_segments(struct whisper_state * state) {
    return state->result_all.size();
}

int whisper_full_lang_id(struct whisper_state * state) {
    return state->lang_id;
}

int64_t whisper_full_get_segment_t0(struct whisper_state * state, int i_segment) {
    return state->result_all[i_segment].t0;
}

int64_t whisper_full_get_segment_t1(struct whisper_state * state, int i_segment) {
    return state->result_all[i_segment].t1;
}

const char * whisper_full_get_segment_text(struct whisper_state * state, int i_segment) {
    return state->result_all[i_segment].text.c_str();
}

int whisper_full_n_tokens(struct whisper_state * state, int i_segment) {
    return state->result_all[i_segment].tokens.size();
}

const char * whisper_full_get_token_text(struct whisper_context * ctx, struct whisper_state * state, int i_segment, int i_token) {
    return ctx->vocab.id_to_token[state->result_all[i_segment].tokens[i_token].id].c_str();
}

whisper_token whisper_full_get_token_id(struct whisper_state * state, int i_segment, int i_token) {
    return state->result_all[i_segment].tokens[i_token].id;
}

struct whisper_token_data whisper_full_get_token_data(struct whisper_state * state, int i_segment, int i_token) {
    return state->result_all[i_segment].tokens[i_token];
}

float whisper_full_get_token_p(struct whisper_state * state, int i_segment, int i_token) {
